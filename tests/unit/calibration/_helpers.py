"""Shared synthetic-data helpers for calibrator tests."""
...
def monotone_loss_table(
    ...
    *,
    ...
    `lambda_max < 1.0` by default (not `1.0`) so losses retain real
    per-example variance at every grid point, including the last one — a
    table ending at an exact zero-variance column is a genuinely
    degenerate case for the KL family's dual (Algorithm Spec F-3), not a
